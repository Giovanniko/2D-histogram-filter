...
class Simulation(object):
	def __init__(self, grid, blur, p_hit,start_pos=None):
		...
		print("incorrect_sense_probablity: " + str(self.incorrect_sense_probability))
		self.colors = self.get_colors()
		self.num_colors = len(self.colors)
		if not start_pos:
			self.true_pose = (self.height/2, self.width/2)
			print("true_pose: " + str(self.true_pose))
		else:
	...
	def move(self, dy, dx):
		...
		print("new_y: " + str(new_y) + ' ' + "new_x: " + str(new_x))
		self.prev_pose = self.true_pose
		print("\nprev_pose (old true_pos): " + str(self.prev_pose))        
		self.true_pose = (new_y, new_x)
		print("\ntrue_pose (updated with new_y and new_x): " + str(self.true_pose))        
		beliefs = deepcopy(self.beliefs)
		print("\nbeliefs in sim.move() going to loc.move(): \n" + str(beliefs))
	...
	def get_observed_color(self):
		...
		print("true_color: " + str(true_color))        
